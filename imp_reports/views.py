...
from .models import LaySp, MasterFinalMistake, UnitBundlereport, FinalPlans,Corarlck1,CoraRollcheck,BuntrackReport,LaySpreadingLayemployee,VueAdGrid1,VueStPunch,StAtt
...
def get_unit_bundle_report_data(request):
    ...
    effective_from = parse_date(start_date) if start_date else date(2026, 9, 6)
...
def unit_bundle(request):
    ...
    effective_from = parse_date(from_date) if from_date else date(2026, 9, 6)
...
def sta(request):
    if request.method == 'GET':
        ...
        data = StAtt.objects.using('demo1').filter(
            date=today
        )
