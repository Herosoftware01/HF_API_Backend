...
def live_scan_data(request):
    ...
    trs_data = (
        ...
        .filter(scan="1", livescan__isnull=True)
    ...
    tob_bottom_map = {
        ...
        for item in MasTopbottom.objects.using("main").filter(
            topbottom_id__in=tbid_list
        )
    }
    ...
    reports = UnitBundlereport.objects.using("app").filter(mbundle_id__in=mbud_list)
    ...
    return JsonResponse(
        {
            ...
            "all_unit_data_count": sum(
                unit["total_records"] for unit in final_data.values()
            ),  # இது 6305 ஆக இருக்கும்
...
def end_live_scan_data(request):
    ...
    for (bundle_id, job_no_key, tb_name_key), info in bundle_info.items():
        ...
        if required_sequences and required_sequences.issubset(
            info["completed_sequences"]
        ):
            unit_id = str(info["unit"])
            unit_data = eligible_units.setdefault(
                unit_id,
                {
                    "unit_id": unit_id,
    ...
    def post(self, request):
        ...
        try:
            with transaction.atomic():
                ...
                bundle_ids = [
                    item.get("bundle_id") for item in data_list if item.get("bundle_id")
                ]
    ...
    def post(self, request):
        ...
        try:
            with transaction.atomic():
                ...
                if bundle_ids:
                    Assembly_data.objects.filter(bundle_id__in=bundle_ids).update(
                        scan=True
                    )
...
def get_eligible_assembly_bundle_ids(job_no, process_des, top_bottom=None):
    ...
    for bundle_id, sequence in completed_rows:
        ...
        if normalized_sequence:
            completed_by_bundle.setdefault(str(bundle_id), set()).add(
                normalized_sequence
            )
...
class GetUnitDataAPIView(APIView):
    def get(self, request):
        ...
        if selected_date:
            ...
            data = unit_input.objects.filter(unit=unit, job_no=job_no).order_by(
                "-entry_date"
            )
        else:
        ...
        if job_no is not None:
            ...
            if process_des is not None:
                ...
                allowed, eligible_bundle_ids, error_message = (
                    get_eligible_assembly_bundle_ids(job_no, process_des, top_bottom)
                )
                if not allowed:
                    return Response(
                ...
                verified_tb_ids = dependency.objects.filter(
                    job_no__iexact=job_no, verify=True
...
class EndUnitDataAPIView(APIView):
    def get(self, request):
        ...
        if selected_date:
            date_obj = datetime.strptime(selected_date, "%Y-%m-%d")
            data = end_line_data.objects.filter(
                unit=unit, line=line, entry_date__date=date_obj
            )
        else:
            # four_days_ago = datetime.now() - timedelta(days=4)
            four_days_ago = date.today()
            data = end_line_data.objects.filter(
                unit=unit, line=line, entry_date__gte=four_days_ago
            )
...
def assembly_emp(request):
    ...
    if not unit or not line or not date:
        return JsonResponse({"error": "unit, line and date are required"}, status=400)
    ...
    line_obj = Line.objects.filter(unit=unit_obj, line_number=line).first()
    ...
    emp_codes = [
        int(emp["emp_code"]) for emp in emp_details if emp["emp_code"].isdigit()
    ]

    emp_names = (
        Empwisesal.objects.using("main")
        .filter(code__in=emp_codes)
        .values("code", "name")
    )

    emp_name_dict = {str(emp["code"]): emp["name"] for emp in emp_names}

    for emp in emp_details:
        emp["emp_name"] = emp_name_dict.get(emp["emp_code"], "")
...
class SaveAssemblyAPIView(APIView):
    def post(self, request):
        ...
        allocation_date = (
            selected_date.date() if selected_date else timezone.now().date()
        )
        unit_obj = Unit.objects.filter(name__iexact=f"unit-{unit}").first()
        line_obj = (
            Line.objects.filter(unit=unit_obj, line_number=line).first()
            if unit_obj
            else None
        )
        ...
        if unit_obj and line_obj:
            ...
            if "top_bottom" in request.data:
                allocation_query = allocation_query.filter(
                    top_bottom__iexact=selected_top_bottom
                )
        ...
        unique_ids = list(
            dict.fromkeys(
                str(value).strip() for value in bundle_ids if str(value).strip()
            )
        )
        if eligible_bundle_ids is not None:
            ineligible_ids = [
                value for value in unique_ids if value not in eligible_bundle_ids
            ]
            if ineligible_ids:
                return Response(
                    {
                        "error": "Previous process is not complete for some bundles.",
                        "bundle_ids": ineligible_ids,
                    },
        ...
        with transaction.atomic():
            ...
            if selected_top_bottom:
                bundle_queryset = bundle_queryset.filter(
                    tb_name__iexact=selected_top_bottom
                )
            ...
            if missing_ids:
                return Response(
                    {
                        "error": "Some bundles are unavailable or already scanned.",
                        "bundle_ids": missing_ids,
                    },
                    status=status.HTTP_409_CONFLICT,
                )
        ...
        return Response(
            {"status": "success", "updated": updated}, status=status.HTTP_200_OK
        )
...
def get_process_details(request):
    ...
    if not jobno or not topbottom:
        return JsonResponse({"error": "Jobno and TopBottom required"}, status=400)

    with connections["demo"].cursor() as cursor:
        cursor.execute("EXEC sp_GetProcessDetails %s, %s", [jobno, topbottom])
    ...
    saved_dependencies = (
        dependency.objects.filter(saved_filter)
    ...
    if latest_by_process:
        ...
        for index, dep in enumerate(
            reversed(list(latest_by_process.values())), start=1
        ):
            saved_result.append(
                {
                    "Jobno": dep.job_no,
                    ...
                    "Trn": trn_map.get(dep.process_id, ""),
                    "Wsec": dep.wsec,
        ...
        try:
            existing_dep = (
                dependency.objects.filter(
                    job_no=job_no, tb_id=tb_id, process_id=process_id
                )
                .order_by("-id")
                .first()
            )
            ...
            if existing_dep:
                ...
                saved_children = existing_dep.data_entries.all().order_by(
                    "desc_ord_no", "id"
                )

                item["saved_selected_processes"] = [
                    child.descriptions for child in saved_children
                ]
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if existing_group.filter(verify=True).exists():
                    return JsonResponse(
                        {"error": "Verified dependency cannot be changed"}, status=409
                    )
                    ...
                    dependency_data.objects.bulk_create(
                        [
                            ...
                            for index, selected in enumerate(
                                selected_processes, start=1
                            )
                        ]
                    )
                    saved_count += 1
            return JsonResponse(
                {"message": "Data saved successfully", "count": saved_count}, status=201
            )
        except Exception as e:
    ...
    try:
        ...
        with transaction.atomic():
            dependencies = dependency.objects.select_for_update().filter(
                job_no=job_no, tb_id=tb_id
            )
            if not dependencies.exists():
                return JsonResponse(
                    {"error": "Save the dependency before verifying"}, status=404
                )
...
def delete_process_dependency(request):
    ...
    try:
        ...
        with transaction.atomic():
            dependencies = dependency.objects.select_for_update().filter(
                job_no=data.get("job_no"), tb_id=data.get("tb_id"), verify=True
            )
            if not dependencies.exists():
                return JsonResponse(
                    {"error": "Verified dependency not found"}, status=404
                )
            deleted_count, _ = dependencies.delete()

        return JsonResponse(
            {"message": "Dependency deleted successfully", "count": deleted_count}
        )
    except Exception as e:
        return JsonResponse({"error": str(e)}, status=400)
...
class UserUnitPermissionView(View):
    def get(self, request):
        ...
        if not user_id:
            return JsonResponse(
                {"success": False, "message": "User is required"}, status=400
            )
        if not app:
            return JsonResponse(
                {"success": False, "message": "App is required"}, status=400
            )
    ...
    def post(self, request):
        try:
            ...
            if app not in ["qcapp", "live_app"]:
                return JsonResponse(
                    {"success": False, "message": "Invalid app"}, status=400
                )
            ...
            valid_unit_ids = list(units.values_list("id", flat=True))

            with transaction.atomic():
                user_unit_permission.objects.filter(user_id=user_id, app=app).delete()
                permission_data = []

                for unit_id in valid_unit_ids:
                    permission_data.append(
                        user_unit_permission(user_id=user_id, app=app, unit_id=unit_id)
                    )
                user_unit_permission.objects.bulk_create(permission_data)
            return JsonResponse(
                {
                    "success": True,
...
class UserUnitPermissionListView(View):
    def get(self, request):
        try:
            ...
            return JsonResponse({"success": True, "results": list(grouped.values())})
        except Exception as e:
            return JsonResponse({"success": False, "message": str(e)}, status=500)
